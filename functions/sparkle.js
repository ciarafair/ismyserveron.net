...
for (var i = 0; i <= sparkCount; i += 1) {
	...
	for (var j = 0; j < sparkParticleCount; j++) {
		...
		var particleSize = particle.random(5, 20);
		var sparkRadius = 20;
	...
	spark.particle = new particle({
		...
		opacity: [
			...
			{ value: 1, duration: 40 },
			{
				value: 0,
				duration: function () {
					return particle.random(500, 800);
				},
			},
		],
...
window.addEventListener(
	"mousemove",
	function (e) {
		particle.set(sparks[sparksIndex].els, {
			left: e.pageX,
			top: e.pageY,
		});
		sparks[sparksIndex].particle.restart();
		sparksIndex++;
